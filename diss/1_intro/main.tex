...
a request to some artificially intelligent system in the ``cloud``, 
...
have enabled extremely large models that 
make these applications possible.
These models have achieved
human-level performance on learning tasks
including image classification~\citep{resnet,alexnet}, image segmentation~\citep{segmentation}, video analysis~\citep{zhang2016video}, text understanding and generation~\citep{bert,gpt}, and have slowly begun to enable more fundamental scientific problems such as protein folding~\citep{protein} drug discovery~\cite{drugdisc}, and medical diagnosis~\cite{diag}.
...
This ``last mile`` takes a few forms.
...
can require large amounts of energy producing high emissions,
and their scale can limit the market
to only large actors with huge existing resources.
...
While existing ``dumb`` or ``natural`` systems that these models aim to assist are not perfect, 
...
These systems are difficult to directly translate to automated machine learning systems,
...
Figure~\ref{fig:dl} illustrates the typical pipeline. 
...
\end{align}
This formulation expands without much change to a multi-dimensional form of the input $x$ and respectively, $w$: the canonical case where a number of features or inputs (e.g., symptoms) are used together to predict the outcome (e.g., diagnosis). 
If we are interested in which dimensions (features) of $x$ are important, we can look at the relative values of the learned ``weights" $w$. In this simple setting, the importance of a feature (say $x^j$) can be exactly determined by the importance of the parameter ($w^j$).
...
Consider a dataset $\{X\}_{i=1}^n$ of size $n$ where each data point in the set $X$ is considered drawn from some underlying distribution over the domain $X \sim \cX^d$, 
with domain dimensionality $d$.
A model $f$ is fit using a parametrizations $\theta \in \Theta$,
with $\Theta$ the space of possible parametrization with some intrinsic dimension $p$. 
...
\end{figure}
\paragraph{Feature Selection.}  methods generally and their applications. structure learning, attention, scan stats...emphasize naive methods being exponential, mention scan stats
...
\end{figure}
\paragraph{Parameter Selection.} methods generally. weight regularization methods, some asymptotics/hypothesis testing guarantees. In DNNs, some hyp testing but mostly indep of classical stats: sparsity, nonlinearity, gating...
...
\section{A specific example: Medical Imaging Analysis}
