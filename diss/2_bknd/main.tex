...
\item Lower case letters will also \todo{dim size notations}

\item \todo{upper case sets, including random variables, and matrix}

%\item $S$ will denote a set of training examples.
%A sample $s_i\in S$ may be indexed by $i \in [1,\ldots,n]$. 
\item \todo{xi, s in S, }%For general and generative settings, $x_i$ may be used in place, and for predictive settings will be represented by the tuple $s_i:=(x_i,y_i)$.

\item \todo{subscripts/superscripts}
